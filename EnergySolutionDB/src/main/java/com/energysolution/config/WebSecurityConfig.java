...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	/*
	 * @Autowired private LoginAuthenticationSuccessHandler LoginAuthSuccessHandler;
	 * 
	 * @Autowired private LoginAuthenticationFailureHandler LoginAuthFailureHandler;
	 * 
	 * @Autowired private AccountProvider accountProvider;
	 * 
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.cors().disable().httpBasic().disable() // rest API 이므로 기본설정 필요 없음
		...
		http.authorizeRequests() // 요청에 대한 권한 지정
				.antMatchers("/", "/Main/**","/images/**").permitAll()
				.antMatchers("/User/**", "/Test/**", "/main/**").permitAll();
