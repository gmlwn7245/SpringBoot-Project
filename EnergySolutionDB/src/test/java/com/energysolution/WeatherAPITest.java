...
public class WeatherAPITest {
	...
	void Test() {
		try{ /*요청정보입력 * 아래와 같은 정보들은 사용자 가이드를 확인하여 찾아주시면 됩니다. * 위도 경도는 엑셀파일 안에 있습니다. * */ //자신이 조회를 원하는 지역의 경도와 위도를 입력해주세요 
			...
			String numOfRows = "10";
			String pageNo = "1";
			String baseDate = "20220513"; // 자신이 조회하고싶은 날짜를 입력해주세요 
			String baseTime = "0600"; //자신이 조회하고싶은 시간대를 입력해주세요 // 서비스 인증키입니다. 공공데이터포털에서 제공해준 인증키를 넣어주시면 됩니다. 
			String serviceKey = "vtjq%2FAY95mysnfmzSx%2FBI0ijID8CYa7fdWqbcRMpeeIV3%2FApXziRmhiSjZ7z1SIhMWKBY%2BC3JAZ6axGiPmt4lQ%3D%3D"; // 정보를 모아서 URL정보를 만들면됩니다. 맨 마지막 "&_type=json"에 따라 반환 데이터의 형태가 정해집니다. 
			String urlStr = "http://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getUltraSrtNcst?" + "serviceKey=" + serviceKey + "&numOfRows="+numOfRows+ "&pageNo="+pageNo+ "&base_date=" + baseDate + "&base_time=" + baseTime + "&nx="+ nx + "&ny=" + ny; 
			...
			bf = new BufferedReader(new InputStreamReader(connection.getInputStream(),"UTF-8")); //버퍼에 있는 정보를 하나의 문자열로 변환. 
			...
			while((line=bf.readLine())!=null){
				...
				JSONParser parser = new JSONParser(); JSONObject obj = (JSONObject) parser.parse(result); // Top레벨 단계인 response 키를 가지고 데이터를 파싱합니다. 
				...
				for(int i = 0 ; i < parse_item.size(); i++) { 
					  ...
					  //String fcscDate =(String)weather.get("fcscDate"); // 출력합니다.
					  System.out.print("배열의 "+i+"번째 요소"); System.out.print(" category : "+category); System.out.print(" fcst_Value : "+ fcst_Value);
					  System.out.println();
